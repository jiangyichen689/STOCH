...
static sgx_status_t SGX_CDECL sgx_foo(void* pms)
{
	...
	char* _tmp_fub = ms->ms_fub;
	size_t _tmp_len = ms->ms_len;
	size_t _len_fub = _tmp_len;
	char* _in_fub = NULL;

	CHECK_REF_POINTER(pms, sizeof(ms_foo_t));
	CHECK_UNIQUE_POINTER(_tmp_fub, _len_fub);

	if (_tmp_fub != NULL) {
		if ((_in_fub = (char*)malloc(_len_fub)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_fub, 0, _len_fub);
	}
	foo(_in_fub, _tmp_len);
err:
	if (_in_fub) {
		memcpy(_tmp_fub, _in_fub, _len_fub);
		free(_in_fub);
	}
