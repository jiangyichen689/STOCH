...
unsigned char * encryptedInt(int num, paillierKeys keys, char delimiter)
{
	...
	int size1 = BN_num_bytes(encrypted_random);
	unsigned char *encrypt_text = (unsigned char *)malloc(size1+1);
	//memset(encrypt_text, 0, size1 + 1);
	BN_bn2bin(encrypted_random, (unsigned char *)encrypt_text);
	//std::string res((char *)encrypt_text);
	//free(encrypt_text);
	//return res;
	encrypt_text[size1] = delimiter;
	return encrypt_text;
}

void makeString(TreeNode* root, std::vector<std::string>& structString, std::vector<std::vector<unsigned char *>>& countString, paillierKeys keys)
{
...
#define key_length_bytes 256
		std::vector<unsigned char*> tempCountString; // [1024 * 2 / 8];
		for (int i = 0; i < size; i++)
		{
			...
			if (tempRoot->child != nullptr)
			{
				...
				if (tempRoot->sibling.size() == 0)
				{
					...
					//unsigned char *tempCountChar = encryptedInt(tempRoot->child->count, keys, ';');// +";";
					tempCountString.push_back(encryptedInt(tempRoot->child->count, keys, ';'));
				}
				else
				{
					tempStructString += tempRoot->child->val + " ";
...
std::unordered_map <char, std::string> myHashMap;
...
int main()
{
	...
	std::ofstream write1;
	std::ofstream write2;
	write1.open("C:\\Users\\user\\Desktop\\GenerateFile\\struct.bin", std::ios::binary);
	write2.open("C:\\Users\\user\\Desktop\\GenerateFile\\count.bin", std::ios::binary);

	QueryPerformanceCounter(&start);
	std::vector<std::string> structString;
	std::vector<std::vector<unsigned char*>> countString;
	...
	QueryPerformanceCounter(&end);
	duration = (double)(end.QuadPart - start.QuadPart) / dFreq;
	printf(" Total encrypted time of counting tree: %f\n", duration);


	QueryPerformanceCounter(&start);
	int len_size = structString.size();
	write1.write((const char*)&len_size, sizeof(int));  // # of level
	write2.write((const char*)&len_size, sizeof(int));  // # of level
	for (int i = 0; i < structString.size(); i++)
	{
		std::string seps = structString[i];
		if(seps.size() % 8 != 0)
		{
			seps += myHashMap['.']; // "0000";
		}
		...
		int str_len = seps.size() / 8;  //4100
		write1.write((const char*)&str_len, sizeof(int));  // [1000][1000][1000][1000][100]  
		std::string output;
		for (int k = 0; k < seps.size(); k += 8)
		{
			output += strtol((const char*)seps.substr(k, 8).c_str(), 0, 2);
		}

		for (int k = 0; k < ((str_len - 1) / max_byte + 1); k++)
		{
			std::string output_tmp = output.substr(k*max_byte, min(max_byte, (str_len- k*max_byte)));
			BN_bin2bn((const unsigned char*)output_tmp.c_str(), output_tmp.size(), random); // this function take bytes as input
			encrypt(encrypted_random, random, &keys.pub, ctx);
			len_size = BN_num_bytes(encrypted_random);
			unsigned char *encrypt_text = (unsigned char *)malloc(len_size);
			BN_bn2bin(encrypted_random, (unsigned char *)encrypt_text);
			write1.write((const char*)&len_size, sizeof(int));
			write1.write((const char*)encrypt_text, len_size);
			free(encrypt_text);
		}

		//write1 << "\r";
		//const char* tempC = countString[i].c_str();
		len_size = countString[i].size();
		write2.write((const char*)&len_size, sizeof(int));
		for (int k = 0; k < countString[i].size(); k++)
		{
			write2.write((const char*)countString[i].at(k), key_length_bytes+1);
		}
		//write2 << "\n";
	}
	///////////////////////////////////////////////////////////////////////
	QueryPerformanceCounter(&end);
	duration = (double)(end.QuadPart - start.QuadPart) / dFreq;
	printf(" Total encrypted time of structure tree: %f\n", duration);
